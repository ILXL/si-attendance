<script>
  ...
  const courses = ["CPSC-120", "CPSC-121", "CPSC-131", "CPSC-240", "MATH-115", "MATH-250"];
  ...
  const signin = (cwid, course) => {
    ...
    response = -1;
    fetch(
      `/signin?cwid=${cwid}&course=${course}`,
  ...
  const nonCWIDSignIn = () => {
    ...
    fetch(
      ...
      `/noncwidsignin?nonCWIDStatus=${non_cwid_status}&course=${course_inp}&name=${non_cwid_name}`,
      {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      }
